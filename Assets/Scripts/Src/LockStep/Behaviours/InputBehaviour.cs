...
namespace LogicFrameSync.Src.LockStep.Behaviours
{
    public class InputBehaviour : ISimulativeBehaviour
    {
        ...
        public void Update()
        {
            ...
            Entitas.Entity entity = Sim.GetEntityWorld().GetEntity((int)GameClientData.SelfPlayer.Id);
            if (entity != null)
            {
                MoveComponent comp = entity.GetComponent<MoveComponent>();
                if (comp != null)
                {
                    if (comp.EntityId == GameClientData.SelfPlayer.Id)
                    {
                        ...
                        if (dir != comp.GetDir())
                        {
                            ...
                            SendKeyFrame(new FrameIdxInfo(ComponentType.MoveComponent, comp.EntityId, new string[] { dir.x + "", dir.y + "", "0" }));
                        }
                    }
                }
            }
